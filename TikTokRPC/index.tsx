...
const STATS_CACHE_KEY = "TikTokRPC.lastStats.v1";
...
function makeFallbackStats(): TikTokStats {
    const username = normalizeUsername(settings.store.username);
    return {
        username,
        following: 34,
...
async function saveStatsCache(stats: TikTokStats) {
    try {
        await DataStore.set(STATS_CACHE_KEY, stats);
    } catch (err) {
        logger.warn("Failed to save TikTok stats cache", err);
    }
}

async function loadStatsCache(): Promise<TikTokStats | undefined> {
    try {
        const cached = await DataStore.get<TikTokStats>(STATS_CACHE_KEY);
        if (!cached) return undefined;

        const following = Number(cached.following);
        const followers = Number(cached.followers);
        const likes = Number(cached.likes);

        if (![following, followers, likes].every(Number.isFinite)) return undefined;

        return {
            ...cached,
            username: normalizeUsername(cached.username || settings.store.username),
            following,
            followers,
            likes,
            profileUrl: cached.profileUrl || `https://www.tiktok.com/@${normalizeUsername(cached.username || settings.store.username)}`
        };
    } catch (err) {
        logger.warn("Failed to load TikTok stats cache", err);
        return undefined;
    }
}

async function getSafeTikTokStats(fetchFresh = true): Promise<TikTokStats> {
    if (!fetchFresh && lastStats) return lastStats;

    try {
        const stats = await fetchTikTokStats();
        lastStats = stats;
        void saveStatsCache(stats);
        return stats;
    } catch (err) {
        logger.error("TikTok stats fetch failed", err);
        ...
        const fallback = makeFallbackStats();
        lastStats = fallback;
        return fallback;
    }
}
    ...
    async updatePresence(fetchStats = true) {
        try {
            if (settings.store.alternateEnabled === true && showingAlternateActivity) {
                ...
                if (fetchStats) {
                    void getSafeTikTokStats(true);
                }

                return;
            }

            const stats = await getSafeTikTokStats(fetchStats);
            safeSetActivity(await createActivity(stats));
        } catch (err) {
            logger.error("TikTokRPC update failed", err);

            showingAlternateActivity = false;
            const stats = lastStats || await loadStatsCache() || makeFallbackStats();
            safeSetActivity(await createActivity(stats));
        }
    }
});
